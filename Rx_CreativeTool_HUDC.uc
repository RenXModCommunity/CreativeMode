class Rx_CreativeTool_HUDC extends Rx_Hud_Component;

var Rx_CreativeTool_ServerFPS ServerFPS;

function Update(float DeltaTime, Rx_HUD HUD) 
{
	local Rx_CreativeTool_ServerFPS thisClass;
	super.Update(DeltaTime, HUD);
	
	if (ServerFPS == None)
	{
		// Find our controller.
		foreach RenxHud.PlayerOwner.AllActors(class'Rx_CreativeTool_ServerFPS', thisClass)
		{
			ServerFPS = thisClass;
			break;
		}
